...
int main() {
    ifstream fin ("q2-input.txt");
    ofstream fout ("q2-output.txt");
    vector<bool> is_prime(10000010, true);
    is_prime[0] = is_prime[1] = false;
    // https://cp-algorithms.com/algebra/sieve-of-eratosthenes.html
    for (int i = 2; i <= 1e7+5; i++) {
        if (is_prime[i] && (long long)i * i <= 1e7+5) {
            for (int j = i * i; j <= 1e7+5; j += i) is_prime[j] = false;
        }
    }
    vector<int> ans(10000010, 0);
    for (int i = 3; i <= 1e7+5; i++) {
        if (is_prime[i] && is_prime[i-2]) ans[i]++;
        if (is_prime[i] && is_prime[i-3]) ans[i]++;
    }
    for (int i = 3; i <= 1e7+5; i++) ans[i] += ans[i-1];
    int t;
    fin >> t;
    for (int o = 1; o <= t; o++) {
        int n;
        fin >> n;
        fout << "Case #" << o << ": " << ans[n] << endl;
    }
    return 0;
}
