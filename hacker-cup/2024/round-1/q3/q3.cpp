...
int main() {
    ifstream fin("q3-input.txt");
    ofstream fout("q3-output.txt");
    ...
    for (int o = 1; o <= t; o++) {
        ...
        long long int result = (p * inv(q, MOD) + MOD) % MOD;
        fout << "Case #" << o << ": " << result << endl;
    }
    return 0;
}
...
Find the expected number of days for a random walk on interval [a, b] where you 
