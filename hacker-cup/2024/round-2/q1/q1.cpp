...
vector<long long int> generate() {
    vector<long long int> nums;
    const int mx_dig = 19;
    const int inc = 2;
    for (int i = 1; i <= mx_dig; i += inc) {
        int k = i / 2;
        for (int j = 1; j <= 9 - k; j++) {
            long long int num = 0;
            for (int l = 0; l <= k; l++) {
...
int prune(vector<long long int> nums, long long int A, long long int B, long long int M) {
    int ans = 0;
    for (int i = 0; i < nums.size(); i++) {
        if (nums[i] >= A && nums[i] <= B && nums[i] % M == 0) ans++;
    }
    return ans;
}
...
int main() {
    ...
    for (int o = 1; o <= t; o++) {
        long long int A, B, M;
        fin >> A >> B >> M;
        vector<long long int> nums = generate();
        int ans = prune(nums, A, B, M);
