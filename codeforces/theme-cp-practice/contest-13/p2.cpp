...
int32_t main() {
    ...
    while (t--) {
        ...
        pair<int, int> v = {1e12, 1e12};
        for (int i = 0; i < k; i++) {
            int v1 = abs(cities[a-1].first - cities[i].first) + abs(cities[a-1].second - cities[i].second);
            int v2 = abs(cities[b-1].first - cities[i].first) + abs(cities[b-1].second - cities[i].second);
            v.first = min(v.first, v1);
            v.second = min(v.second, v2);
        }
        int v3 = abs(cities[a-1].first - cities[b-1].first) + abs(cities[a-1].second - cities[b-1].second);
        int ans = min(v.first + v.second, v3);
        debug(ans);
        cout << ans << endl;
    }
}
...
Take minimum between two major cities going to a or going to b. As the shortest 
path between a and b will just be 1 step and not travel to any non-major city.
