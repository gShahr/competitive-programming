...
Minimum sum will always be equal to taking the bitwise and of the entire array. Answer can be more than 1 if and only if array bitwise and
is 0 which in that case go from left to right as we want the least amount of elements per group to maximize the number of groups.

Whoops griefing on this because we can always extend our group of zeros all the way to the right if it doesn't form a group of its own.
