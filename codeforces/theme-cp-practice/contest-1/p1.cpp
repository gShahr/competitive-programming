...
((a+b)/2 + c)/2 = (a+b)/4 + c/2
=> This doesn't really work.abort
To prove we start from the average and then add a-1 where a is the average of the array back into the array in order
to generate original array.

Nevermind to what is above - it is actually a fallacious statement as showing the average can be made into an identical array
does not necessarily prove that it is the best way to generate the answer. Instead, we want to forgoe using the greatest element
so the midpoint is the greatest possible. Do this for whole array and can prove using induction.
