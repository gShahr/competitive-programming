...
int32_t main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int choice;
            if (curr.size() <= 1) choice = curr[0];
            else choice = curr[1];
            choices.push_back(choice);
        }
        sort(choices.begin(), choices.end());
        int ans = accumulate(choices.begin(), choices.end(), 0LL) - choices[0] + mn;
        debug(ans);
        cout << ans << endl;
    }
}
...
Answer is to take second smallest element partition of n-1 elements (assuming there are 2 or more elements) which is the greatest 
and add it to the minimum element across all elements. 
