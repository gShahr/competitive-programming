    ...
    while (t--) {
        ...
        do {
            ...
            for (int i = 0; i < 3; i++) {
                int index = perm[i];
                int best = n-1;
                while (used.find(mx[index][best].second) != used.end()) best--;
                used.insert(mx[index][best].second);
                curr += mx[index][best].first;
            }
            ans = max(ans, curr);
...
We can brute force the answer by picking the maximum choice for each permutation of n=3 since we can only pick one choice for each activity.
