...
int32_t main() {
    ...
    while (t--) {
        ...
        ans = max(v1, v2);
        bool pos = false;
        for (auto i: a) if (i > 0) pos = true;
        if (!pos) ans = *max_element(a.begin(), a.end());
        debug(ans);
        cout << ans << endl;
    }
}
...
group of 3 positives => ??
group of 3+ negatives => get rid

Stategy: Get rid of negatives first and then take max of remaining postives
in even/odd parity. 

Nevermind, above strategy actually fails I think because taking local maxima 
is better so that particle addition isn't used for elements we want to remove
later.
...
odd length => 1 remaining negative
even length => 2 reminaing negatives
