...
We want to spread minimum numbers as much as possible, so split it up into k parts and then fill in rest of array with left over numbers.
Putting a number at position will make it in the group for k-1, k-2, k-3, so forth and so on such that k-x >= 1 in both directions left
and right. So to minimize this we put min in the spot that maximizes the number of groups it is in. If the minimum is that spot, we don't 
need another until k distance away since all other positions are covered by first minimum assuming we go from left to right.
