...
int sol(int n, int m, vector<int>& a) {
    const int mx = 5001;
    int dp[mx][mx];
    memset(dp, -1, sizeof(dp));
    function<int(int, int, int)> countWays = [&](int round, int strength, int intelligence) -> int {
        int res = 0;
        if (round == n) return 0;
        else if (a[round] > 0) {
            res = countWays(round + 1, strength, intelligence);
            if (intelligence >= a[round]) res++;
        } else if (a[round] < 0) {
            res = countWays(round + 1, strength, intelligence);
            if (strength >= abs(a[round])) res++;
        } else {
            if (dp[strength][intelligence] != -1) return dp[strength][intelligence];
            res = max(countWays(round + 1, strength + 1, intelligence),
                      countWays(round + 1, strength, intelligence + 1));
        }
        return dp[strength][intelligence] = res;
    };
