...
int32_t main() {
    ...
    while (t--) {
        ...
        for (auto i: a) check[i] = true;
        vector<int> unused;
        for (int i = 1; i <= n; i++) {
            if (!check[i]) unused.push_back(i);
        }
        vector<int> b(n);
        map<int, int> freq;
        for (int i = 0; i < n; i++) {
...
The idea is to make sure the element appears more than anything else in the prefix that we are looking at (hence the mode definition).
There is a simple solution that relies on each element appearing at most once which means that this is the constraint of the solution
where the algo follows.

Algo:
Follow idea below with a vector of unused elements. Unused elements generated by taking vector of booleans and taking inverse of those and
putting that into an array.
First time showing up => use
Repeated times showing up => use an element that is not used at all in array a
