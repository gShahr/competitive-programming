...
long long int dfs(vector<long long int>&a, set<long long int>& visited, map<long long int, vector<long long int>>& adj,  long long int x) {
    long long int ans = x;
    if (visited.find(x) != visited.end()) return ans;
    visited.insert(x);
    for (auto i: adj[x]) {
        ans = max(ans, dfs(a, visited, adj, i));
    }
    return ans;
}
...
5 4 4 5 1
5 5 6 8 5 => values needed to use index
=> 5.{5, 6, 9}
