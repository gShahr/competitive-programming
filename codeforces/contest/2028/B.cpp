...
dx are the number of elements that we need to use
n - dx are the number of elements that we can freely use
similarly n - dx represents the amount of numbers we need to fill in
...
dx := number of elements strictly less than n <=> < n
=> n = 100, b = 2, c = 1
=> There are n elements with being the position of the first element 
=> It will look something like this [1, x]
=> x-c = (x-1) / 2 + 1 = (x-c)/b + 1

Find greatest x such that c + bx < n 
=> set c + bx = n-1 => (n-c-1)/b = x
100-1 = 99 => 99 / 2 = 49
