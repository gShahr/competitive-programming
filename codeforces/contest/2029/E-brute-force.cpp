...
void dfs(int x, vector<bool>& q) {
    if (x >= q.size() || q[x]) return;
    q[x] = true;
    vector<int> d;
    for (int i = 1; i * i <= x; i++) {
        if (x % i == 0) {
...
int32_t main() {
    ...
    while (t--) {
        ...
        if (p.size() <= 0) ans = 2;
            ...
            bool valid = true;
            int mx = 4*1e5+1;
            vector<bool> q(mx, false);
            for (int j = 2; j <= 5000; j ++) {
                if (is_prime(j)) {
                    string curr = "PRIME: " + to_string(j) + ": ";
                    cout << curr << ' ';
                    vector<bool> qq(100000, false);
                    dfs(j, qq);
                    for (int i = 2*j; i <= 4*j; i++) {
                        if (!qq[i] && !is_prime(i)) cout << i << ' ';
                    }
                    cout << endl;
                }
            }
