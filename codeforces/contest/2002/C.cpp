...
int main() {
    ...
    while (t--) {
        ...
        cin >> xs >> ys >> xt >> yt;
        long long int distSquared = (xt - xs) * (xt - xs) + (yt - ys) * (yt - ys);
        bool ok = true;
        for (int i = 0; i < n; i++) {
            long long int x = a[i].first, y = a[i].second;
            long long int currDistSquared = (x - xt) * (x - xt) + (y - yt) * (y - yt);
            debug(currDistSquared, distSquared);
            if (currDistSquared <= distSquared) {
                ok = false;
            }
        }
        debug(ok);
