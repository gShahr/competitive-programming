...
int32_t main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            int v1 = a[i];
            auto it = lower_bound(b.begin(), b.end(), mn + a[i]);
            int bj = *it;
            if (it == b.end() && v1 < mn) {
                ok = false;
                break;
...
Main idea => Make right index as big as possible or make left index as small
as possible going in that direction and taking min / max of original value
or modified value.
...
Copied over solution for C2 because the harder version encapsulates the easier 
version of the problem fully which just allows me to submit twice and get to
AC for solving one meta-problem.
