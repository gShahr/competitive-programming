...
void sol() {
    ...
    if (x != y) {
        ...
        cout << e << ' ' << s << endl;
        for (int i = s; i >= e; i--) {
            if (a[i] == '1' && k == 0) k |= (1LL << (63-i));
            else if (a[i] == '0') k |= (1LL << (63-i));
        }
        string c = bitset<64>(k).to_string();
...
+00111
 ...
 Go from right to left (most significant) and pick the first instance where 
 we get 1 in string a and 0 in string b. String a is the string choose
 when making all its bits except one of them zero.

 Since k is bounded by 10^18 while x and y are bounded by 10^9, make k large.
