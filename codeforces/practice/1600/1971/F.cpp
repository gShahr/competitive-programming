...
int main() {
    ...
    for (int i = 0; i < t; i++) {
        ...
        for (int i = 0; i <= r; i++) {
            long long int x = i;
            long long int lower = ceil(sqrt(r*r - x*x));
            long long int upper = ceil(sqrt((r+1)*(r+1) - x*x));
            long long int res = 0;
            if (x <= 0) res = 2 * (upper - lower);
            else if (lower > 0) res = 4 * (upper - lower);
            else res = 2 * (2 * (upper - lower) - 1);
            ans += res;
            debug(x, lower, upper, res);
        }
        cout << ans << endl;
        debug(ans);
    }
