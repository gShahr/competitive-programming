...
int32_t main() {
    ...
    while (t--) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            int curr = (currMx * binpow(2, twoCnt, mod) + preSum) % mod;
            ans.push_back(curr);
        }
        debug(ans);
...
Simple algo
=> Take largest number each time and multiply by max amount of times prime factor 2 appears, use binary exponentiation 
under mod m to multiply it and add sum of all previous numbers together.
