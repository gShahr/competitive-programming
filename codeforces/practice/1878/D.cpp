...
int main() {
    ...
    while (t--) {
        ...
        int q;
        cin >> q;
        vector<bool> moves(n, false);
        while (q--) {
            int x;
            cin >> x;
            x--;
            int pos = lower_bound(r.begin(), r.end(), x) - r.begin();
            int a = min(x, r[pos] + l[pos] - x);
            int b = max(x, r[pos] + l[pos] - x);
            moves[a] = moves[a] ^ true;
            moves[b] = moves[b] ^ true;
        }
        debug(moves);
        bool flip = false;
        for (int i = 0; i < n; i++) {
            flip = flip ^ moves[i];
            int pos = lower_bound(r.begin(), r.end(), i) - r.begin();
            // cout << r[pos] + l[pos] - i << ' ' << pos << endl;
            if (flip && i <= r[pos] + l[pos] - i) swap(s[i], s[r[pos] + l[pos] - i]);
        }
        cout << s << endl;
    }
}
